"""
HAIAMM Model Classes

This module defines the domain model for the Human-Assisted Intelligence Assurance Maturity Model (HAIAMM).
It extends the OpenSAMM structure with multi-domain support for comprehensive HAI security assessment.

The model supports 6 domains: Software, Infrastructure, Endpoints, Data, Processes, and Vendors.
Each domain contains 4 business functions with security practices and maturity levels.
"""

import json
import os
from dataclasses import dataclass, field
from typing import List, Optional, Dict
from pathlib import Path


@dataclass
class AssessmentCriteria:
    """
    Represents a single assessment question for a maturity level.

    Attributes:
        id: Unique identifier for the question (e.g., "sm_software_l1_q1")
        question: The assessment question text
        guidance: Additional guidance to help answer the question
    """
    id: str
    question: str
    guidance: str = ""

    @classmethod
    def from_dict(cls, data: Dict) -> 'AssessmentCriteria':
        """Create AssessmentCriteria from dictionary."""
        return cls(
            id=data.get('id', ''),
            question=data.get('question', ''),
            guidance=data.get('guidance', '')
        )

    def to_dict(self) -> Dict:
        """Convert to dictionary for serialization."""
        return {
            'id': self.id,
            'question': self.question,
            'guidance': self.guidance
        }


@dataclass
class MaturityLevel:
    """
    Represents a single maturity level (1, 2, or 3) within a security practice.

    Attributes:
        level: The maturity level number (1, 2, or 3)
        objective: High-level objective for this maturity level
        activities: List of activities to achieve this level
        assessment_criteria: List of assessment questions for this level
    """
    level: int
    objective: str
    activities: List[str] = field(default_factory=list)
    assessment_criteria: List[AssessmentCriteria] = field(default_factory=list)

    @classmethod
    def from_dict(cls, data: Dict) -> 'MaturityLevel':
        """Create MaturityLevel from dictionary."""
        criteria = [
            AssessmentCriteria.from_dict(q)
            for q in data.get('assessment_criteria', [])
        ]
        return cls(
            level=data.get('level', 0),
            objective=data.get('objective', ''),
            activities=data.get('activities', []),
            assessment_criteria=criteria
        )

    def to_dict(self) -> Dict:
        """Convert to dictionary for serialization."""
        return {
            'level': self.level,
            'objective': self.objective,
            'activities': self.activities,
            'assessment_criteria': [c.to_dict() for c in self.assessment_criteria]
        }

    def get_question_count(self) -> int:
        """Get the number of assessment questions for this level."""
        return len(self.assessment_criteria)


@dataclass
class SecurityPractice:
    """
    Represents a security practice within a business function.

    Each practice has 3 maturity levels with assessment criteria.

    Attributes:
        id: Short identifier (e.g., "sm", "pc", "ta")
        name: Full name of the practice (e.g., "Strategy & Metrics")
        short_name: Abbreviated name (e.g., "SM")
        objective: Overall objective of this practice
        description: Detailed description
        maturity_levels: List of 3 maturity levels
    """
    id: str
    name: str
    short_name: str
    objective: str
    description: str
    maturity_levels: List[MaturityLevel] = field(default_factory=list)

    @classmethod
    def from_dict(cls, data: Dict) -> 'SecurityPractice':
        """Create SecurityPractice from dictionary."""
        levels = [
            MaturityLevel.from_dict(level_data)
            for level_data in data.get('maturity_levels', [])
        ]
        return cls(
            id=data.get('id', ''),
            name=data.get('name', ''),
            short_name=data.get('short_name', ''),
            objective=data.get('objective', ''),
            description=data.get('description', ''),
            maturity_levels=levels
        )

    def to_dict(self) -> Dict:
        """Convert to dictionary for serialization."""
        return {
            'id': self.id,
            'name': self.name,
            'short_name': self.short_name,
            'objective': self.objective,
            'description': self.description,
            'maturity_levels': [l.to_dict() for l in self.maturity_levels]
        }

    def get_level(self, level_num: int) -> Optional[MaturityLevel]:
        """
        Get a specific maturity level.

        Args:
            level_num: Level number (1, 2, or 3)

        Returns:
            MaturityLevel if found, None otherwise
        """
        for level in self.maturity_levels:
            if level.level == level_num:
                return level
        return None

    def get_questions_for_level(self, level_num: int) -> List[AssessmentCriteria]:
        """
        Get all assessment questions for a specific maturity level.

        Args:
            level_num: Level number (1, 2, or 3)

        Returns:
            List of AssessmentCriteria for the level
        """
        level = self.get_level(level_num)
        return level.assessment_criteria if level else []

    def get_total_questions(self) -> int:
        """Get total number of assessment questions across all levels."""
        return sum(level.get_question_count() for level in self.maturity_levels)


@dataclass
class BusinessFunction:
    """
    Represents a business function within a domain.

    Each function contains security practices.

    Attributes:
        id: Identifier (e.g., "governance", "engineering", "verification", "operations")
        name: Full name (e.g., "Governance", "Engineering")
        description: Description of the business function
        practices: List of SecurityPractice objects
    """
    id: str
    name: str
    description: str
    practices: List[SecurityPractice] = field(default_factory=list)

    @classmethod
    def from_dict(cls, data: Dict) -> 'BusinessFunction':
        """Create BusinessFunction from dictionary."""
        practices = [
            SecurityPractice.from_dict(practice_data)
            for practice_data in data.get('practices', [])
        ]
        return cls(
            id=data.get('id', ''),
            name=data.get('name', ''),
            description=data.get('description', ''),
            practices=practices
        )

    def to_dict(self) -> Dict:
        """Convert to dictionary for serialization."""
        return {
            'id': self.id,
            'name': self.name,
            'description': self.description,
            'practices': [p.to_dict() for p in self.practices]
        }

    def get_practice(self, practice_id: str) -> Optional[SecurityPractice]:
        """
        Get a specific security practice by ID.

        Args:
            practice_id: Practice identifier (e.g., "sm", "pc")

        Returns:
            SecurityPractice if found, None otherwise
        """
        for practice in self.practices:
            if practice.id == practice_id:
                return practice
        return None


@dataclass
class Domain:
    """
    Represents one of the 6 HAIAMM domains.

    Each domain contains 4 business functions with security practices.

    Attributes:
        id: Identifier (e.g., "software", "infrastructure", "endpoints")
        name: Full name (e.g., "Software Domain")
        description: Description of the domain
        business_functions: List of 4 BusinessFunction objects
    """
    id: str
    name: str
    description: str
    business_functions: List[BusinessFunction] = field(default_factory=list)

    @classmethod
    def from_dict(cls, data: Dict) -> 'Domain':
        """Create Domain from dictionary."""
        functions = [
            BusinessFunction.from_dict(func_data)
            for func_data in data.get('business_functions', [])
        ]
        return cls(
            id=data.get('id', ''),
            name=data.get('name', ''),
            description=data.get('description', ''),
            business_functions=functions
        )

    def to_dict(self) -> Dict:
        """Convert to dictionary for serialization."""
        return {
            'id': self.id,
            'name': self.name,
            'description': self.description,
            'business_functions': [f.to_dict() for f in self.business_functions]
        }

    def get_function(self, function_id: str) -> Optional[BusinessFunction]:
        """
        Get a specific business function by ID.

        Args:
            function_id: Function identifier (e.g., "governance", "engineering")

        Returns:
            BusinessFunction if found, None otherwise
        """
        for function in self.business_functions:
            if function.id == function_id:
                return function
        return None

    def get_practice(self, practice_id: str) -> Optional[SecurityPractice]:
        """
        Get a specific practice across all business functions.

        Args:
            practice_id: Practice identifier (e.g., "sm", "pc")

        Returns:
            SecurityPractice if found, None otherwise
        """
        for function in self.business_functions:
            practice = function.get_practice(practice_id)
            if practice:
                return practice
        return None

    def get_all_practices(self) -> List[SecurityPractice]:
        """Get all practices in this domain."""
        practices = []
        for function in self.business_functions:
            practices.extend(function.practices)
        return practices

    def get_total_questions(self) -> int:
        """Get total number of questions in this domain."""
        return sum(
            practice.get_total_questions()
            for practice in self.get_all_practices()
        )


class HAIAMMModel:
    """
    Complete HAIAMM (Human-Assisted Intelligence Assurance Maturity Model) representation.

    Contains 6 domains, each with 4 business functions and multiple security practices.
    Extends OpenSAMM v1.0 with multi-domain support for comprehensive HAI security assessment.

    Attributes:
        version: Model version (e.g., "1.0")
        model_name: Full model name
        description: Model description
        domains: List of 6 Domain objects
    """

    def __init__(self):
        """Initialize empty HAIAMM model."""
        self.version: str = "1.0"
        self.model_name: str = ""
        self.description: str = ""
        self.domains: List[Domain] = []
        self._domain_index: Dict[str, Domain] = {}
        self._practice_index: Dict[str, Dict[str, SecurityPractice]] = {}  # domain_id -> practice_id -> practice

    def load_from_config(self, config_path: Optional[str] = None):
        """
        Load HAIAMM model from JSON configuration file.

        Args:
            config_path: Path to JSON config file. If None, uses default location.

        Raises:
            FileNotFoundError: If config file doesn't exist
            json.JSONDecodeError: If config file is invalid JSON
        """
        if config_path is None:
            # Default to config/haiamm_multi_domain_data.json
            module_dir = Path(__file__).parent.parent
            config_path = module_dir / "config" / "haiamm_multi_domain_data.json"

        if not os.path.exists(config_path):
            raise FileNotFoundError(f"HAIAMM config file not found: {config_path}")

        with open(config_path, 'r', encoding='utf-8') as f:
            data = json.load(f)

        self.version = data.get('version', '1.0')
        self.model_name = data.get('model_name', '')
        self.description = data.get('description', '')

        # Load domains
        self.domains = [
            Domain.from_dict(domain_data)
            for domain_data in data.get('domains', [])
        ]

        # Build indices for fast lookup
        self._build_indices()

    def _build_indices(self):
        """Build internal indices for fast lookup."""
        self._domain_index = {}
        self._practice_index = {}

        for domain in self.domains:
            self._domain_index[domain.id] = domain
            self._practice_index[domain.id] = {}

            for function in domain.business_functions:
                for practice in function.practices:
                    self._practice_index[domain.id][practice.id] = practice

    def get_domain(self, domain_id: str) -> Optional[Domain]:
        """
        Get a domain by its ID.

        Args:
            domain_id: Domain identifier (e.g., "software", "infrastructure")

        Returns:
            Domain if found, None otherwise
        """
        return self._domain_index.get(domain_id)

    def get_practice(self, domain_id: str, practice_id: str) -> Optional[SecurityPractice]:
        """
        Get a security practice by domain and practice ID.

        Args:
            domain_id: Domain identifier (e.g., "software")
            practice_id: Practice identifier (e.g., "sm", "pc")

        Returns:
            SecurityPractice if found, None otherwise
        """
        domain_practices = self._practice_index.get(domain_id, {})
        return domain_practices.get(practice_id)

    def get_all_practices_for_domain(self, domain_id: str) -> List[SecurityPractice]:
        """
        Get all practices for a specific domain.

        Args:
            domain_id: Domain identifier

        Returns:
            List of SecurityPractice objects
        """
        domain = self.get_domain(domain_id)
        return domain.get_all_practices() if domain else []

    def get_all_practices(self) -> List[SecurityPractice]:
        """
        Get all practices across all domains.

        Returns:
            List of all SecurityPractice objects from all domains
        """
        practices = []
        for domain in self.domains:
            practices.extend(domain.get_all_practices())
        return practices

    def get_domain_count(self) -> int:
        """Get total number of domains (should be 6)."""
        return len(self.domains)

    def get_total_practices(self) -> int:
        """Get total number of practices across all domains."""
        return sum(len(domain.get_all_practices()) for domain in self.domains)

    def get_total_questions(self) -> int:
        """Get total number of assessment questions across all domains."""
        return sum(domain.get_total_questions() for domain in self.domains)

    def get_domain_names(self) -> List[str]:
        """Get list of all domain names."""
        return [domain.name for domain in self.domains]

    def get_domain_ids(self) -> List[str]:
        """Get list of all domain IDs."""
        return [domain.id for domain in self.domains]

    @property
    def business_functions(self) -> List['BusinessFunction']:
        """
        Get business functions for compatibility with OpenSAMMModel interface.

        Since all domains share the same 4 business function structure,
        this returns the business functions from the first domain.

        Returns:
            List of BusinessFunction objects
        """
        if self.domains:
            return self.domains[0].business_functions
        return []

    def generate_full_questionnaire_structure(self) -> Dict:
        """
        Generate a complete questionnaire structure for all domains.

        Returns:
            Dictionary with questionnaire structure organized by domain, function, and practice
        """
        questionnaire = {
            'version': self.version,
            'model_name': self.model_name,
            'total_domains': self.get_domain_count(),
            'total_questions': self.get_total_questions(),
            'domains': []
        }

        for domain in self.domains:
            domain_data = {
                'id': domain.id,
                'name': domain.name,
                'description': domain.description,
                'functions': []
            }

            for function in domain.business_functions:
                func_data = {
                    'id': function.id,
                    'name': function.name,
                    'description': function.description,
                    'practices': []
                }

                for practice in function.practices:
                    practice_data = {
                        'id': practice.id,
                        'name': practice.name,
                        'short_name': practice.short_name,
                        'objective': practice.objective,
                        'levels': []
                    }

                    for level in practice.maturity_levels:
                        level_data = {
                            'level': level.level,
                            'objective': level.objective,
                            'questions': [
                                {
                                    'id': q.id,
                                    'question': q.question,
                                    'guidance': q.guidance
                                }
                                for q in level.assessment_criteria
                            ]
                        }
                        practice_data['levels'].append(level_data)

                    func_data['practices'].append(practice_data)

                domain_data['functions'].append(func_data)

            questionnaire['domains'].append(domain_data)

        return questionnaire

    def to_dict(self) -> Dict:
        """Convert entire model to dictionary for serialization."""
        return {
            'version': self.version,
            'model_name': self.model_name,
            'description': self.description,
            'domains': [d.to_dict() for d in self.domains]
        }

    def __repr__(self) -> str:
        """String representation of the model."""
        return (
            f"HAIAMMModel(version={self.version}, "
            f"domains={self.get_domain_count()}, "
            f"total_practices={self.get_total_practices()}, "
            f"total_questions={self.get_total_questions()})"
        )


# Module-level function for easy loading
def load_haiamm_model(config_path: Optional[str] = None) -> HAIAMMModel:
    """
    Convenience function to load and return a HAIAMM model.

    Args:
        config_path: Optional path to config file

    Returns:
        Loaded HAIAMMModel instance

    Example:
        >>> model = load_haiamm_model()
        >>> print(f"Total domains: {model.get_domain_count()}")
        Total domains: 6
        >>> print(f"Domain names: {', '.join(model.get_domain_names())}")
    """
    model = HAIAMMModel()
    model.load_from_config(config_path)
    return model


if __name__ == "__main__":
    # Test the model loading
    try:
        model = load_haiamm_model()
        print(f"Loaded: {model}")
        print(f"\nDomains:")
        for domain in model.domains:
            print(f"  - {domain.name} ({len(domain.business_functions)} business functions, "
                  f"{len(domain.get_all_practices())} practices, "
                  f"{domain.get_total_questions()} questions)")
            for func in domain.business_functions:
                print(f"      {func.name}: {len(func.practices)} practices")
    except Exception as e:
        print(f"Error loading model: {e}")
        import traceback
        traceback.print_exc()
